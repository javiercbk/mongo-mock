...
module.exports = function Collection(db, state) {
  ...
  var interface = {
    ...
    findOneAndUpdate: function (filter, update, options, callback) {
      ...
      asyncish(function () {
        self.findOne(filter, options, function (err, object) {
          if (err) {
            callback(err);
          } else if (!object){
            if (!opts.upsert) {
              ...
              self.insert({}, function(err, newObj) {
                if (err) {
                  callback(err);
                }
                self.updateOne({ _id: newObj._id }, update, function() {
                  //naive way to retrieve the new value;
                  self.findOne({ _id: object._id }, callback);
                });
              });
            }
          } else {
            var properCallback = function () {
              //naive way to retrieve the new value;
              self.findOne({ _id: object._id }, callback);
            };
            if (opts.returnOriginal === false) {
            } else {
              properCallback = function (err) {
                if (err) {
                  callback(err);
                } else {
                  callback(null, obj);
            ...
            self.updateOne({ _id: object._id }, update, properCallback);
          }
        });
      });
      if(typeof callback!=='function') {
